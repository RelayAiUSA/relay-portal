...
// for Zoho Books and QuickBooks. Used by oauth-token.js, oauth-refresh-sweep.js,
// and accountingSync.js.
...
// Call this right before making an accounting-API request. Returns a ready
// decrypted access token (refreshing first if within 5 min of expiring), or
// null if the connection is missing/broken -- callers should treat null as
// 'skip the sync, this customer needs to reconnect' rather than throwing.
// Also returns realmId (QuickBooks company id) and organizationId (Zoho Books
// org id, if already cached on the token doc) so callers don't need a
// second Firestore read.
async function ensureFreshToken(db, uid, provider) {
const tokenRef = db.collection('users').doc(uid).collection('oauth_tokens').doc(provider);
let tokenSnap = await tokenRef.get();
if (!tokenSnap.exists) return null;

let tokenData = tokenSnap.data();
const expiresAtMs = tokenData.expiresAt ? tokenData.expiresAt.toMillis() : 0;
const isStale = expiresAtMs - Date.now() < 5 * 60 * 1000;
...
module.exports = { encrypt, decrypt, refreshCustomerToken, ensureFreshToken, PROVIDER_REFRESH };
